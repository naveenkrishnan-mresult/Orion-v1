...
if st.session_state.step == "hlr":
        ...
        if selected:
            ...
            with st.spinner("Retrieving project tasks..."):
                ...
                for ind,issue in enumerate(issues):
                    issues_detail.append(f"{ind}. Issue: {issue.key} - {issue.summary} <br> Type: {issue.issue_type} <br> Status: {issue.status} <br> {f'Description: {issue.description} <br>' if issue.description else ''}<br>")
                issues_detail_str = " <br> ".join(issues_detail)
                st.session_state.workflow_state["issues_detail"] = issues_detail_str
                st.session_state.workflow_state["selected_issues"] = [issue.key for issue in issues]
            
            bot_message(f"Found {len(issues)} issues in project {project_key}</b><br>{issues_detail_str}")
    ...
    with st.spinner("Analyzing requirement and generating questions..."):
