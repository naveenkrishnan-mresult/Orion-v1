...
# Move imports to the top to avoid redundant imports
...
if "step" not in st.session_state:
    st.session_state.step = "start_choice"
...
# Chat Dialog
@st.dialog("ORION Chat", width="large")
def chat_interface():
    @st.cache_resource
    ...
    # Display chat history with workflow steps integrated
    with st.container():
    ...
    # Ensure user input is captured in every session
    # Refactor workflow steps to include input fields and proper state management

    # Add user input for the start_choice step
    if st.session_state.step == "start_choice":
        st.markdown("<b>Choose an option:</b>", unsafe_allow_html=True)
        col1, col2 = st.columns(2)
        with col1:
            if st.button("Start New Project Workflow", use_container_width=True):
                user_message("Start New Project Workflow")
                bot_message("Let me fetch your JIRA projects...")
                st.session_state.step = "jira_projects"
                st.rerun()
        with col2:
            if st.button("View History & Resume Session", use_container_width=True):
                user_message("View History & Resume Session")
                st.session_state.step = "history_menu"
                st.rerun()

    # Add user input for the history_menu step
    elif st.session_state.step == "history_menu":
        ...
        if not sessions:
            bot_message("No previous sessions found. Starting fresh workflow...")
            st.session_state.step = "jira_projects"
            if "sessions_loaded" in st.session_state:
                del st.session_state.sessions_loaded
            st.rerun()
        else:
            st.markdown("<b>Previous Sessions:</b>", unsafe_allow_html=True)
            
            for i, session in enumerate(sessions, 1):
                if f"summary_{i}" not in st.session_state:
                    st.session_state[f"summary_{i}"] = st.session_state.history_manager.get_agent_summary(session)
                summary = st.session_state[f"summary_{i}"]
                col1, col2 = st.columns([4, 1])
                
                with col1:
                    if st.button(f"{i}. {summary}", key=f"session_{i}", use_container_width=True):
                        # Load session state
                        state = st.session_state.history_manager.get_session_state(session['session_id'])
                        if state:
                            # Update workflow state with loaded session
                            st.session_state.workflow_state.update(state)
                            st.session_state.workflow_state["is_resumed"] = True
                            
                            user_message(f"Resuming session: {session['session_id']}")
                            
                            # Get resume suggestion
                            suggestion = st.session_state.history_manager.get_resume_suggestion(state)
                            bot_message(f"Session loaded! {suggestion}")
                            
                            # Determine next step based on current state
                            current_step = state.get('current_step', '')
                            if current_step == 'analyze_requirements':
                                st.session_state.step = "qa"
                            elif current_step == 'setup_generation':
                ...
                with col2:
                    if st.button("🗑️", key=f"delete_{i}", help="Delete session"):
                        ...
                        for j in range(1, len(sessions) + 1):
                            if f"summary_{j}" in st.session_state:
                                del st.session_state[f"summary_{j}"]
                        st.rerun()
            
            if st.button("Start New Workflow", use_container_width=True):
                user_message("Start New Workflow")
                # Clear cached data
                if "sessions_loaded" in st.session_state:
                    del st.session_state.sessions_loaded
                for j in range(1, len(sessions) + 1):
        ...
        if not st.session_state.projects:
            ...
            if selected:
                ...
                # Save checkpoint
                st.session_state.history_manager.save_checkpoint(st.session_state.workflow_state)
        ...
        if idx < len(questions):
            ...
            if user_input:
                user_message(user_input)
                st.session_state.workflow_state["responses"][q.id] = user_message
        ...
        if st.button("🔄 Start New Workflow"):
            ...
            for key in list(st.session_state.keys()):
                if key not in ['step', 'messages', 'workflow_state', 'agents', 'agents_initialized']:
    ...
    # Handle user input for start step
    if st.session_state.step == "hlr":
        if user_input:
            ...
            if any([i in user_input.lower() for i in keyword]):
                show_typing_with_response("Choose how you'd like to start:", "start_choice")
            else:
...
# Modify the chat container CSS to enable auto-scroll
st.markdown("""
<style>
.chat-container {
    height: 400px;
    ...
    width: 100%; /* Make the chat container cover the page width */
}
</style>
""", unsafe_allow_html=True)

# Add JavaScript to ensure the chat container scrolls to the latest message
st.markdown("""
<script>
